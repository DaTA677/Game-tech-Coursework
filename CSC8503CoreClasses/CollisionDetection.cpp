...
bool CollisionDetection::RayBoxIntersection(const Ray& r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	Vector4 intersection = rayPos + (rayDir * bestT);
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	...
	if (((a->GetIgnoreLayer() & b->GetLayer()) != 0) || ((b->GetIgnoreLayer() & a->GetLayer()) != 0) || !a->IsActive() || !b->IsActive())
		return false;
...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	if (overlap) {
		...
		float distances[6] = {
			(maxB.x - minA.x),(maxA.x - minB.x),(maxB.y - minA.y),(maxA.y - minB.y),(maxB.z - minA.z),(maxA.z - minB.z)
		};
...
bool  CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	newTransform.SetPosition(localSpherePos);

	AABBVolume newVolume = AABBVolume(volumeA.GetHalfDimensions());
	

	bool collided = AABBSphereIntersection(newVolume, newTransformA, volumeB, newTransform, collisionInfo);

	if (collided) {
		collisionInfo.point.normal = orientation * collisionInfo.point.normal;
		collisionInfo.point.localB= orientation * collisionInfo.point.localB + position;
		//collisionInfo.point.localA = orientation * collisionInfo.point.localA + position;

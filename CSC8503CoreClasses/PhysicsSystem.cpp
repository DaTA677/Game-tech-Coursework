...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 newPositionA = transformA.GetPosition() - (p.normal * p.penetration * (physA->GetInverseMass() / totalMass));
	Vector3 newPositionB = transformB.GetPosition() + (p.normal * p.penetration * (physB->GetInverseMass() / totalMass));
	
	transformA.SetPosition(newPositionA);
	transformB.SetPosition(newPositionB);
	...
	Vector3 fullImpulse = p.normal * j * physA->friction;
	//if (fullImpulse.x < 0.0001f && fullImpulse.y < 0.0001f && fullImpulse.z < 0.0001f) {
	//	physA->SetLinearVelocity(Vector3());
	//	physB->SetLinearVelocity(Vector3());
	//	physA->SetAngularVelocity(Vector3());
	//	physB->SetAngularVelocity(Vector3());
	//	//physA->ClearForces();
	//	//physB->ClearForces();
	//	return;
	//}
...
void PhysicsSystem::SpringResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	physA->ApplyLinearImpulse(-p.normal*(p.penetration*springConstant));
	physB->ApplyLinearImpulse(p.normal * (p.penetration * springConstant));

	physA->ApplyAngularImpulse(Vector3::Cross(p.localA, -p.normal * (p.penetration * springConstant)));
	physB->ApplyAngularImpulse(Vector3::Cross(p.localB, p.normal * (p.penetration * springConstant)));
}
